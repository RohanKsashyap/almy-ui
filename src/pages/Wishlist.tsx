      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10">
          ...
          {wishlistProducts.length > 0 && (
            <button 
              ...
              className="flex items-center gap-2 bg-[#DC2626]/10 text-[#DC2626] px-6 py-3 rounded-full font-bold text-sm hover:bg-[#DC2626] hover:text-white transition-all shadow-sm"
            >
        ...
        {wishlistProducts.length > 0 ? (
                ...
                <div key={productId} className="group relative">
                  <div className="relative aspect-[3/4] rounded-[2.5rem] overflow-hidden bg-gray-50 mb-4 shadow-sm group-hover:shadow-xl transition-all duration-500">
                    ...
                    <div className="absolute top-4 left-4 flex flex-col gap-2">
                      {status === 'Coming Soon' ? (
                        ...
                        <span className="px-3 py-1 bg-red-100/90 backdrop-blur-md text-[#DC2626] text-[10px] font-bold rounded-full uppercase tracking-widest">
                          Limited Quantity
                        </span>
                      ) : (
                  ...
                  <div className="px-2">
                    <div className="flex justify-between items-start mb-1">
                      <h3 className="font-bold text-gray-900 group-hover:text-[#DC2626] transition-colors line-clamp-1">
                        {product.name}
                      </h3>
                    ...
                    {status === 'Coming Soon' ? (
                      ...
                      <button 
                        ...
                        className="w-full py-4 bg-white text-[#DC2626] border-2 border-[#DC2626]/10 rounded-2xl text-xs font-bold uppercase tracking-widest hover:bg-[#DC2626] hover:text-white hover:border-[#DC2626] transition-all"
                      >
                        Add to Bag
                      </button>
                    )}
          ...
          <div className="bg-white rounded-[3rem] p-12 md:p-20 text-center border border-dashed border-gray-200 mb-20">
            <div className="w-20 h-20 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-6 text-[#DC2626]">
            ...
            <Link 
              to="/shop" 
              className="inline-flex items-center gap-2 bg-[#DC2626] text-white px-8 py-4 rounded-2xl font-bold uppercase tracking-widest text-sm hover:bg-[#B91C1C] transition-all shadow-lg shadow-red-100"
            >
              Start Shopping
            </Link>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 md:gap-8">
              {suggestedProducts.map((product) => (
                <div key={(product._id || product.id) as string | number} className="group cursor-pointer" onClick={() => navigate(`/product/${product.slug || product._id || product.id}`)}>
                  ...
                  <h3 className="font-bold text-gray-900 text-sm mb-1 group-hover:text-[#DC2626] transition-colors truncate">
                    {product.name}
                  </h3>
