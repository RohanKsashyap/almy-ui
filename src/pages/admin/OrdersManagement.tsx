    ...
    <div className="space-y-8 animate-fadeIn" onClick={() => setOpenDropdownId(null)}>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-red-50 flex items-center justify-center text-red-500 shrink-0 shadow-sm">
            <Sparkles size={24} />
          </div>
          <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative flex-1 md:w-80" onClick={e => e.stopPropagation()}>
            ...
            <input 
              ...
              className="w-full bg-white border border-gray-200 rounded-2xl pl-11 pr-4 py-2.5 text-sm focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all outline-none"
            />
          </div>
          {/* <button className="bg-[#DC2626] hover:bg-[#B91C1C] text-white px-5 py-2.5 rounded-2xl text-sm font-bold flex items-center gap-2 transition-all shadow-lg shadow-red-500/20">
            <Plus size={18} />
            Create Order
          </button> */}
        </div>
        ...
        {tabs.map((tab) => (
          <button 
            ...
            className={`whitespace-nowrap px-6 py-2.5 rounded-2xl text-sm font-bold transition-all ${
              activeTab === tab 
                ? 'bg-[#DC2626] text-white shadow-md shadow-red-500/10' 
      ...
      <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-visible">
            ...
            <tbody className="divide-y divide-gray-50">
              {filteredOrders.length > 0 ? (
                filteredOrders.map((order) => (
                  <tr key={order._id} className={`group hover:bg-gray-50/50 transition-colors ${selectedOrders.includes(order._id) ? 'bg-red-50/30' : ''}`}>
        ...
        <div className="px-8 py-6 bg-white border-t border-gray-50 flex items-center justify-between" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex items-center gap-1">
            ...
            <button className="w-8 h-8 rounded-lg bg-[#DC2626] text-white text-[11px] font-bold shadow-md shadow-red-500/20">1</button>
        ...
        <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 space-y-4">
          <div>
            ...
            <div className="flex items-baseline gap-2 mt-2">
              ...
              <span className="text-red-500 text-xs font-bold">Real-time</span>
            </div>
          </div>
        </div>
