        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input 
              ...
              className="pl-12 pr-6 py-3 bg-white border border-gray-100 rounded-2xl w-full md:w-80 text-sm focus:ring-2 focus:ring-red-500/20 focus:border-red-500 outline-none transition-all shadow-sm"
              value={searchTerm}
          ...
          <button 
            onClick={onAdd}
            className="flex items-center gap-2 bg-[#DC2626] hover:bg-[#B91C1C] text-white px-6 py-3 rounded-2xl text-sm font-bold transition-all shadow-lg shadow-red-200"
          >
            <Plus size={18} />
            Add New Category
          </button>
      ...
      <div className="flex items-center gap-8 border-b border-gray-100 overflow-x-auto">
        <button 
          ...
          className={`pb-4 text-sm font-bold transition-all relative whitespace-nowrap ${
            activeTab === 'all' ? 'text-red-600' : 'text-gray-400 hover:text-gray-600'
          }`}
        >
          All Categories
          {activeTab === 'all' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-red-600" />}
        </button>
        <button 
          onClick={() => setActiveTab('archived')}
          className={`pb-4 text-sm font-bold transition-all relative whitespace-nowrap ${
            activeTab === 'archived' ? 'text-red-600' : 'text-gray-400 hover:text-gray-600'
          }`}
        >
          Archived
          {activeTab === 'archived' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-red-600" />}
        </button>
        <button 
          onClick={() => setActiveTab('recent')}
          className={`pb-4 text-sm font-bold transition-all relative whitespace-nowrap ${
            activeTab === 'recent' ? 'text-red-600' : 'text-gray-400 hover:text-gray-600'
          }`}
        >
          Recently Added
          {activeTab === 'recent' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-red-600" />}
        </button>
      </div>
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      {cat.imageUrl ? (
                        ...
                        <div className="w-10 h-10 rounded-xl bg-red-50 flex items-center justify-center text-red-500 border border-red-100 shadow-sm">
                          <LayoutGrid size={20} />
                        </div>
                      )}
