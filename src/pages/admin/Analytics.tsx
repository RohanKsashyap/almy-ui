...
const MetricCard = ({ title, value, change, isPositive }: { title: string, value: string, change: string, isPositive: boolean }) => (
  <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 transition-all hover:shadow-md hover:border-red-100 group">
...
const RevenueChart = ({ data = [] }: { data?: any[] }) => {
  ...
  return (
    <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 h-full group hover:border-red-100 transition-all">
  ...
  return (
    <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 h-full group hover:border-red-100 transition-all">
      ...
      <div className="flex items-end justify-between gap-6 h-48 mb-10">
        {data.map((c, i) => (
          <div key={i} className="flex-1 flex flex-col items-center gap-4 h-full group/bar">
            <div className="w-full bg-gray-50 rounded-2xl relative h-full overflow-hidden border border-gray-50 group-hover/bar:border-red-50 transition-all">
              <div 
                className={`absolute bottom-0 w-full ${i === 0 ? 'bg-[#DC2626]' : 'bg-[#111827]'} rounded-t-xl transition-all duration-1000 ease-out delay-${i * 100}`} 
      ...
      <div className="flex items-center justify-between pt-8 border-t border-gray-50">
        ...
        <span className="text-[10px] font-black text-[#DC2626] uppercase tracking-[0.2em] bg-red-50 px-3 py-1 rounded-full">{data[0]?._id || 'N/A'}</span>
      </div>
    </div>
  );
};
...
const TopSellingProducts = ({ products = [] }: { products?: any[] }) => {
  return (
    <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 group hover:border-red-100 transition-all">
        ...
        {products.map((p, i) => (
          <div key={i} className="flex items-center justify-between p-4 rounded-2xl border border-transparent hover:border-gray-50 hover:bg-gray-50/50 transition-all group/item">
            <div className="flex items-center gap-5">
              ...
              <div>
                <h4 className="text-sm font-black text-gray-900 group-hover/item:text-[#DC2626] transition-colors">{p.name}</h4>
      ...
      <div className="space-y-8 relative">
        ...
        {orders.map((o, i) => (
          <div key={i} className="flex items-start gap-6 relative group/activity">
            <div className={`w-12 h-12 rounded-2xl bg-white border-2 border-red-50 flex items-center justify-center flex-shrink-0 z-10 group-hover/activity:border-[#DC2626] group-hover/activity:bg-red-50 transition-all duration-500`}>
              <ShoppingBag size={20} className="text-[#DC2626]" />
            </div>
            <div className="pt-1">
        ...
        <div>
          <div className="flex items-center gap-3 mb-4">
            <div className="flex -space-x-2">
              {[1, 2, 3].map(i => (
                <div key={i} className="w-6 h-6 rounded-full border-2 border-white bg-red-100 flex items-center justify-center text-[8px] font-black text-red-500 uppercase">EB</div>
              ))}
            </div>
            <div className="flex items-center gap-2 px-3 py-1 bg-red-50 rounded-full border border-red-100">
              ...
              <span className="text-[9px] font-black text-[#DC2626] uppercase tracking-widest">Live System Active</span>
            </div>
          </div>
          <h2 className="text-5xl font-black text-gray-900 tracking-tighter leading-tight">Analytics <span className="text-[#DC2626]">Command Center.</span></h2>
