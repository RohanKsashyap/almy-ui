    ...
    <DashboardLayout>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-8 md:mb-10">
        <div>
          <h1 className="text-2xl md:text-4xl font-bold text-gray-900 mb-2">
            Welcome back, <span className="text-[#DC2626]">{user.fullName?.split(' ')[0] || 'User'}</span>!
          </h1>
        ...
        <div className="bg-white p-6 rounded-[2rem] md:rounded-[2.5rem] border border-gray-50 shadow-sm relative group overflow-hidden">
          <div className="flex justify-between items-start mb-6">
            ...
            <button 
              ...
              className="text-[10px] font-bold text-[#DC2626] uppercase tracking-wider hover:underline"
            >
              View All
            </button>
          </div>
      ...
      <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 border border-gray-50 shadow-sm mb-8 md:mb-10">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          ...
          <button className="text-sm font-semibold text-[#DC2626] hover:underline flex items-center gap-2">
            Download Report
          </button>
        </div>
      ...
      <div>
        <div className="flex justify-between items-center mb-6">
          ...
          <Link to="/wishlist" className="text-sm font-bold text-[#DC2626] hover:underline flex items-center gap-1">
        ...
        {wishlistProducts.length > 0 ? (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            {latestWishlist.map((product) => (
              <div key={product._id || product.id} className="group cursor-pointer">
                <div className="relative aspect-[3/4] rounded-2xl md:rounded-3xl overflow-hidden mb-3" onClick={() => navigate(`/product/${product.slug || product._id || product.id}`)}>
                  <img 
                    src={product.image} 
                    alt={product.name}
                    className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                  />
                  <button className="absolute top-4 right-4 w-8 h-8 bg-white/80 backdrop-blur-md rounded-full flex items-center justify-center text-[#DC2626] shadow-sm">
                    <Heart className="w-4 h-4 fill-current" />
                  </button>
                </div>
                <h3 className="font-bold text-gray-900 text-sm mb-1 group-hover:text-[#DC2626] transition-colors line-clamp-1">
                  {product.name}
                </h3>
          ...
          <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-8 md:p-12 text-center border border-dashed border-gray-200">
            ...
            <Link to="/shop" className="text-[#DC2626] font-bold mt-2 inline-block">Start Shopping</Link>
          </div>
        )}
      </div>
    </DashboardLayout>
