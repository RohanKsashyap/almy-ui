...
export default function NotFound() {
  const popularCategories = [
    ...
    { name: 'Party Dresses', href: '/shop?type=party', icon: <ShoppingBag className="w-5 h-5" /> },
    { name: 'Special Occasion', href: '/shop?type=special', icon: <Heart className="w-5 h-5" /> },
    ...
    <main className="min-h-screen bg-white flex items-center justify-center py-20 px-6">
      ...
      <div className="max-w-2xl w-full text-center">
        ...
        <p className="text-gray-600 text-lg mb-12 max-w-md mx-auto">
          It seems the page you're looking for has wandered off. Don't worry, there's plenty more to discover in our store.
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-12">
          {popularCategories.map((cat) => (
            <Link
              key={cat.name}
              to={cat.href}
              className="flex items-center justify-between p-6 bg-gray-50 rounded-2xl border border-transparent hover:border-hot-pink/30 hover:bg-red-50/30 transition-all group"
            >
