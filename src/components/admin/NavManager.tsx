    ...
    <div className="space-y-12">
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="flex gap-4">
          ...
          <button 
            className="px-10 py-3.5 bg-[#DC2626] text-white rounded-2xl text-sm font-bold hover:bg-[#B91C1C] transition-all shadow-lg shadow-red-500/20"
            onClick={() => onSave(nav)}
          >
            Save Navigation
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr,360px] gap-12 items-start">
        <div className="space-y-8">
              ...
              <div className="p-8 bg-[#FAFAFA]/50">
                {c.items.length > 0 ? (
                  <div className="space-y-4">
                    {c.items.map((it, ii) => (
                      <div key={ii} className="flex gap-4 items-center bg-white border border-[#F3F4F6] rounded-2xl p-5 shadow-sm group transition-all hover:border-red-100">
                    ...
                    <button 
                      className="w-full py-4 border-2 border-dashed border-[#E5E7EB] rounded-2xl text-xs font-bold uppercase tracking-widest text-[#9CA3AF] hover:border-[#DC2626] hover:text-[#DC2626] transition-all flex items-center justify-center gap-2 mt-4" 
                      onClick={() => addItem(ci)}
                    >
                      <Plus size={16} />
                      Add Sub-item
          ...
          <button 
            className="w-full py-8 border-2 border-dashed border-[#D1D5DB] rounded-[2rem] text-sm font-black uppercase tracking-[0.2em] text-[#9CA3AF] hover:border-[#DC2626] hover:text-[#DC2626] hover:bg-red-50/30 transition-all flex flex-col items-center justify-center gap-4 group"
            onClick={addCategory}
          >
            <div className="w-12 h-12 rounded-2xl bg-gray-50 flex items-center justify-center group-hover:bg-red-100 transition-all">
              <Plus size={24} />
            </div>
        ...
        <div className="bg-white border border-[#F3F4F6] rounded-[2.5rem] shadow-xl shadow-gray-200/50 sticky top-8 overflow-hidden">
          ...
          <div className="p-8">
            <div className="flex flex-col items-center mb-12">
              <span className="text-2xl font-black tracking-tighter">LUXE<span className="text-[#DC2626]">.</span></span>
            </div>
                ...
                <div key={idx} className="space-y-3">
                  ...
                  {idx === 0 && c.items.length > 0 && (
                    <div className="pl-4 space-y-3 border-l-2 border-[#F3F4F6]">
                      {c.items.map((it, ii) => (
                        <p key={ii} className="text-sm font-medium text-[#6B7280] hover:text-[#DC2626] cursor-pointer transition-colors">
                          {it.label || 'New Item'}
                        </p>
                      ))}
                    </div>
      ...
      <div className="pt-12 border-t border-[#F3F4F6] flex justify-between items-center text-[11px] font-bold uppercase tracking-widest text-[#9CA3AF]">
        ...
        <div className="flex gap-8">
          <a href="#" className="hover:text-[#DC2626] transition-colors">Documentation</a>
          <a href="#" className="hover:text-[#DC2626] transition-colors">Support</a>
        </div>
      </div>
    </div>
  );
