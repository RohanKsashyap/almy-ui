        ...
        <div className="w-full md:w-[380px] bg-[#FDFBFB] p-8 md:p-12 flex flex-col border-r border-gray-100 overflow-y-auto custom-scrollbar">
          <div className="flex-1 space-y-12">
            <div className="text-center md:text-left">
              <div className="w-24 h-24 rounded-[2rem] bg-gradient-to-tr from-red-100 to-red-50 flex items-center justify-center text-red-600 font-bold text-3xl border-4 border-white shadow-xl mx-auto md:mx-0 mb-6">
              ...
              <div className="inline-flex items-center px-4 py-1.5 rounded-full bg-red-100 text-[#DC2626] text-[10px] font-black uppercase tracking-widest">
                <span className="w-2 h-2 rounded-full bg-[#DC2626] mr-2"></span>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-4 group">
                    <div className="p-2 bg-white rounded-xl shadow-sm border border-gray-50 text-red-500">
                      <Mail size={16} />
                    </div>
                    <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {customer.addresses && customer.addresses.length > 0 ? (
                  customer.addresses.map((addr: any, idx: number) => (
                    <div key={idx} className={`p-6 rounded-3xl border ${addr.isPrimary ? 'border-red-100 bg-red-50/20' : 'border-gray-100 bg-gray-50/30'} space-y-3`}>
                      <div className="flex justify-between items-start">
                        <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">{addr.type || 'Address'}</span>
                        {addr.isPrimary && (
                          <span className="px-2 py-0.5 bg-red-500 text-white text-[8px] font-black uppercase tracking-widest rounded-md">Primary</span>
                        )}
                      </div>
              ...
              <div className="space-y-4">
                {customer.orders && customer.orders.length > 0 ? (
                  customer.orders.map((order: any, idx: number) => (
                    <div key={idx} className="group p-6 rounded-[2rem] bg-[#FDFBFB] border border-gray-100 hover:border-red-200 hover:shadow-xl hover:shadow-red-500/5 transition-all duration-300">
                      <div className="flex flex-col sm:flex-row justify-between gap-6">
                        ...
                        <div className="flex sm:flex-col justify-between items-end gap-2">
                          ...
                          <button 
                            ...
                            className="flex items-center gap-1.5 text-[9px] font-black text-red-600 uppercase tracking-widest group-hover:gap-2 transition-all"
                          >
