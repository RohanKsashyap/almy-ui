      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="flex flex-col items-center justify-center mb-8">
          <label className="relative group cursor-pointer">
            <div className={`w-24 h-24 rounded-full border-2 border-dashed flex flex-col items-center justify-center transition-all overflow-hidden ${
              preview ? 'border-[#DC2626]' : 'border-gray-200 group-hover:border-red-300'
            }`}>
              {preview ? (
                <img src={preview} className="w-full h-full object-cover" alt="Avatar preview" />
              ) : (
                <>
                  <Camera className="text-gray-300 group-hover:text-red-400 mb-1" size={24} />
        ...
        <div className="space-y-4">
          <div className="space-y-1.5">
            ...
            <input 
              className="w-full bg-gray-50 border-none rounded-2xl px-5 py-3.5 text-sm font-bold placeholder:text-gray-300 focus:ring-2 focus:ring-red-100 transition-all" 
          ...
          <div className="space-y-1.5">
            ...
            <select 
              className="w-full bg-gray-50 border-none rounded-2xl px-5 py-3.5 text-sm font-bold text-gray-700 focus:ring-2 focus:ring-red-100 transition-all appearance-none" 
              value={form.product} 
            ...
            <div className="flex gap-2 bg-gray-50 rounded-2xl px-5 py-3.5">
              {[1, 2, 3, 4, 5].map((n) => (
                ...
                >
                  <Star
                    size={20}
                    className={`${n <= form.rating ? 'text-[#DC2626] fill-[#DC2626]' : 'text-gray-200'}`}
                  />
                </button>
              ))}
            </div>
          ...
          <div className="space-y-1.5">
            ...
            <textarea 
              className="w-full bg-gray-50 border-none rounded-2xl px-5 py-4 text-sm font-medium placeholder:text-gray-300 focus:ring-2 focus:ring-red-100 transition-all h-32 resize-none" 
